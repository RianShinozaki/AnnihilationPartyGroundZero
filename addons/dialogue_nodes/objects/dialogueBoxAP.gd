...
func _enter_tree():
	...
	margin_container.add_child(options)
...
func proceed(idx: String):
	...
	match(type):
		...
		'6':
			# condition
			var result = _skill_check((dialogue_data.nodes[idx]))
...
## Resets the speaker & dialogue of the DialogueBox.
func reset():
	speaker.text = ''
	dialogue.text = ''
...
func _set_dialogue(dict):
	...
	dialogue.get_v_scroll_bar().hide()
...
func _skill_check(skill_dict: Dictionary):
